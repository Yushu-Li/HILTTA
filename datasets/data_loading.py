...
# from datasets.pacs import get_pacs_datasets


logger = logging.getLogger(__name__)


def get_transform(dataset_name, adaptation, size=32):
    """
...
def get_test_loader(arch, setting, adaptation, dataset_name, root_dir, domain_name, severity, num_examples,
                    domain_names_all, rng_seed, alpha_dirichlet=0, batch_size=128, shuffle=False, workers=4,domain_id=0):
    ...
    if 'cifar' in dataset_name and 'vit_' in arch:
        transform = get_transform(dataset_name, adaptation, 224)
    else:
    ...
    if domain_name == "none":
        ...
        if dataset_name in {"cifar10_c", "cifar100_c"}:
            ...
            if dataset_name in {"imagenet_d", "imagenet_d109"}:
                ...
                for dom_name in domain_names_all:
                    if not os.path.exists(os.path.join(data_dir, dom_name)):
                        ...
                        domainnet_dir = os.path.join(root_dir + '/imagenet-d', dom_name)
            ...
            test_dataset = ImageList(image_root=data_dir,
                                     ...
                                     transform=transform)
        # elif dataset_name == "pacs" or dataset_name == "office-home":
            
        #     test_dataset = get_pacs_datasets(data_dir, domain_name)
            # raise NotImplementedError("PACS is not supported yet!")
        else:
            raise ValueError(f"Dataset '{dataset_name}' is not supported!")

    try:
        if 'nonoverlap_continual' in setting:
            ...
            # get the subset of the test dataset
            test_dataset.samples = test_dataset.samples[domain_id::len(domain_names_all)]
            logger.info(f"Successfully created non-overlapping test dataset for domain {domain_id} with {len(test_dataset)} samples")
...
def get_source_loader(dataset_name, root_dir, adaptation, batch_size, train_split=True, ckpt_path=None, num_samples=None, percentage=1.0, workers=4, size=32):
    ...
    transform = get_transform(dataset_name, adaptation, size=size)
    ...
    if dataset_name == "cifar10":
        ...
        # source_dataset = torchvision.datasets.ImageNet(root=data_dir,
                                                    #    split=split,
                                                    #    transform=transform)
        source_dataset = torchvision.datasets.ImageFolder(data_dir+'/val/', transform)
        ...
        # data_dir = '/Data03/liyushu/tta_datasets/ImageNet-D'

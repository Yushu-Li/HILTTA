...
class CustomResNet(torch.nn.Module):
    def __init__(self, original_model, mean,
    std):
        super(CustomResNet, self).__init__()
        
        self.features = torch.nn.Sequential(*list(original_model.children())[:-2]) 
        self.norm = ImageNormalizer(mean, std)
...
def get_torchvision_model(model_name, weight_version="IMAGENET1K_V1"):
    ...
    # for getting feature
    # model = CustomResNet(model, transform.mean, transform.std)
...
class TransformerWrapper_cifar(torch.nn.Module):
    def __init__(self, model):
        super().__init__()
        # self.model = model
        self.__dict__ = model.__dict__.copy()
...
def get_model(cfg, num_classes):
    if cfg.CORRUPTION.DATASET == "domainnet126":
        ...
        base_model = resnet.get_resnet_model('pacs',50, 7)
        checkpoint = torch.load("/cluster/personal/code/Active_TTA/ttab-main/pretrain/ckpt/ttab_ckpts/pacs/resnet50_bn_ssh_photo.pth", map_location="cpu")
        # import ipdb; ipdb.set_trace()
        base_model.load_state_dict(checkpoint['model'])
        base_model.fc = base_model.classifier
        # base_model = normalize_model(base_model, (0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))

    elif cfg.CORRUPTION.DATASET == "office-home":
        base_model = resnet.get_resnet_model('pacs',50, 65)
        checkpoint = torch.load("/cluster/personal/code/Active_TTA/ttab-main/pretrain/ckpt/ttab_ckpts/officehome/resnet50_bn_ssh_realworld.pth", map_location="cpu")
        # import ipdb; ipdb.set_trace()
        base_model.load_state_dict(checkpoint['model'])
        base_model.fc = base_model.classifier
        # base_model = normalize_model(base_model, (0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
        
    elif cfg.MODEL.ARCH == "vit_b_16" and  "cifar10" in cfg.CORRUPTION.DATASET:
        print("load vit_b_16 model for cifar")
        config = CONFIGS['ViT-B_16']
        num_classes = 10 if cfg.CORRUPTION.DATASET == "cifar10_c" else 100
        base_model = VisionTransformer(config, 224, zero_head=False, num_classes=num_classes)
        if cfg.CORRUPTION.DATASET == "cifar10_c":
            base_model.load_state_dict(torch.load(cfg.CKPT_DIR+'/ViT-cifar/cifar10-100_500_checkpoint.bin'))
            ...
            base_model.load_state_dict(torch.load(cfg.CKPT_DIR+'/ViT-cifar/cifar100_500_checkpoint.bin'))
            # model.load_from(np.load(cfg.CKPT_DIR+'/ViT-cifar/cifar100_500_checkpoint.bin'))
        base_model.cuda()
        ...
        try:
            ...
            print('successfully loaded model from torchvision')
        except ValueError:
            try:
                # load model from timm
                base_model = get_timm_model(cfg.MODEL.ARCH)
                print('successfully loaded model from timm')
            except ValueError:
                try:
                    ...
                    elif cfg.MODEL.ARCH == "resnet26_10":
                        base_model = resnet.get_resnet_model('cifar10',26, 10)
                        checkpoint = torch.load("/cluster/personal/code/Active_TTA/ttab-main/pretrain/ckpt/ttab_ckpts/cifar10/resnet26_bn_ssh_cifar10.pth", map_location="cpu")
                        # import ipdb; ipdb.set_trace()
                        base_model.load_state_dict(checkpoint['model'])
                        base_model.fc = base_model.classifier
                        base_model = normalize_model(base_model, (0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
                    ...
                    print('successfully loaded model from robustbench')
...
def split_up_model(model, arch_name, dataset_name):
    ...
    if hasattr(model, "model") and hasattr(model.model, "pretrained_cfg") and hasattr(model.model, model.model.pretrained_cfg["classifier"]):
        ...
        classifier = model.fc
    elif dataset_name in {"pacs", "office-home"}:
        ...
        classifier = model.model.fc
    elif "vit_" in arch_name and "cifar" in dataset_name:
        # import ipdb; ipdb.set_trace()
        encoder = TransformerWrapper_cifar(model)

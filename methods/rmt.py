...
class RMT(TTAMethod):
    def __init__(self, cfg, model, num_classes):
        ...
        if cfg.MODEL.ARCH=="vit_b_16" and 'cifar' in self.dataset_name:
            size=224
        else:
            size=32
        _, self.src_loader = get_source_loader(dataset_name=cfg.CORRUPTION.DATASET,
                                               ...
                                               workers=min(cfg.SOURCE.NUM_WORKERS, os.cpu_count()),size=size)
        ...
        # split up the model
        if arch_name=="vit_b_16" and 'cifar' in self.dataset_name:
            self.feature_extractor = self.model.transformer
